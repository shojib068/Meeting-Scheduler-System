...
<?php
  ...
  $query = "SELECT cm.id, cm.place,cm.title,cm.date, cm.time,cm.description, GROUP_CONCAT(am.attendee_mail SEPARATOR '  
       ||     ') AS attendees,GROUP_CONCAT(a.topic SEPARATOR ' || ')AS topic, GROUP_CONCAT(a.description SEPARATOR ' || ') AS description, GROUP_CONCAT(a.decision SEPARATOR ' || ') AS decision, a.description,a.topic,a.decision
...
if (isset($_POST['submit'])) {
  ...
  ||     ') AS attendees, a.description,a.topic,a.decision
    ...
    if (mysqli_num_rows($q) == 0) {
        echo "Sorry! No such type meeting found found. Try searching again.";
    } else {
      echo "<table><tr>
      <th>Meeting ID</th>
      </tr>";
        ...
        ?>
        <form method="POST">
        <button type="submit" name="add_agenda" style="background-color:#101010ed;color:green;font-weight:700;" class="btn btn-default"> <span style="color:green;" class="glyphicon glyphicon-ok-sign"></span><a href="edit_agenda.php">&nbsp Edit Agenda</a> </button></form>
  <?php  }
}

?>

<a href="generate-pdf.php"><input type="submit" value="generate MOM" name="submit"  id="submit"></a>
